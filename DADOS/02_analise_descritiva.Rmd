...
caminho_dados_finais <- "C:/Users/ferna/OneDrive/Área de Trabalho/UFG/Projeto Prob e Estatistica/Projeto_Probabilidade_e_Estat-sica/DADOS/enem_2024_GOIAS_filtrado_final.csv"
...
### Análise: Nota da Redação (`NU_NOTA_REDACAO`)

```{r analise-continua-nota-redacao, echo=FALSE}
# 1. Calcular estatísticas descritivas da nota de redação
summary_stats_red <- summary(dados_enem$NU_NOTA_REDACAO, na.rm = TRUE)
media_red <- round(mean(dados_enem$NU_NOTA_REDACAO, na.rm = TRUE), 2)
mediana_red <- median(dados_enem$NU_NOTA_REDACAO, na.rm = TRUE)
q1_red <- quantile(dados_enem$NU_NOTA_REDACAO, 0.25, na.rm = TRUE)
q3_red <- quantile(dados_enem$NU_NOTA_REDACAO, 0.75, na.rm = TRUE)
sd_red <- round(sd(dados_enem$NU_NOTA_REDACAO, na.rm = TRUE), 2)

# 2. Gráfico: Histograma
hist(dados_enem$NU_NOTA_REDACAO,
     main="Gráfico X: Distribuição das Notas de Redação (GO)",
...
# 3. Gráfico: Boxplot
boxplot(dados_enem$NU_NOTA_REDACAO,
        main="Gráfico Y: Boxplot das Notas de Redação (GO)",
...
# Layout compacto com par(mfrow): Boxplots de todas as áreas do ENEM

# Define layout: 2 linhas, 3 colunas
par(mfrow = c(2, 3))

# Boxplot - Ciências da Natureza
boxplot(dados_enem$NU_NOTA_CN,
        main = "Ciências da Natureza",
        ylab = "Nota",
        col = "darkgreen")

# Boxplot - Ciências Humanas
boxplot(dados_enem$NU_NOTA_CH,
        main = "Ciências Humanas",
        ylab = "Nota",
        col = "darkblue")

# Boxplot - Linguagens
boxplot(dados_enem$NU_NOTA_LC,
        main = "Linguagens",
        ylab = "Nota",
        col = "darkorange")
